...
# This Code is meant to include similarity between two different versions of a package based on relative difference method.
...
class RelativeSimilarity(object):
    ...
    def filter_package(self, input_stack, ref_stacks):					# Function applies Jaccard Similarity at Package Name only
        ...
        for ref_stack in ref_stacks:
            ...
            original_score = RelativeSimilarity.compute_modified_jaccard_similarity(len(input_set), len(refstack_component_list), vcount);
            if original_score > jaccard_threshold:
                filetered_ref_stacks.append(ref_stack)
        return filetered_ref_stacks

    def find_relative_similarity(self,input_stack, input_stack_vectors, filtered_ref_stacks):					# Function for Relative Similarity Similarity
        input_set = set(list(input_stack.keys()))
        ...
        for ref_stack in filtered_ref_stacks:
            ...
            for component, ref_stack_component_version in zip(refstack_component_list, corresponding_version):
                if component in input_stack:
                    ...
                    if(self.compare_version (ref_stack_component_version, input_component_version)):
                        ...
                        vcount = vcount + self.getp_value_graph(component, input_stack_vectors, ref_stack)
                else:
            ...
            boosted_score, missing_downstream_component =  self.downstream_boosting(missing_package_list,ref_stack, 
            ...
            if(original_score > similarity_score_threshold):
                ...
                similar_stack = SimilarStack(objid, usage_score, source, original_score, downstream_score, missing_package_list, version_mismatch_list, missing_downstream_component)
