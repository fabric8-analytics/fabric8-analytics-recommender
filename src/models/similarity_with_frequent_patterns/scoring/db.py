...
#TODO  - change the url as configurtion parameter
def get_reference_stacks_from_graph( list_packages ):
    ...
    payload = {'gremlin': "g.V().hasLabel('Version').has('pname', within("+str_packages+")).in('StackVersion').dedup().valueMap(true);"}
    ...
    if refstacks is not None:
        for refstack in refstacks:
            ...
            map_ref_stack['application_description'] = "Generated Stack: " + refstack.get('sname')[0]
            ...
            if((len(components) >3 and len(components) <9)):
                ...
                for component in components:
                    ...
                    package['code_complexity'] = float(component.get('code_complexity')[0]) if ('code_complexity' in component) else 0
                    package['is_downstream'] = component.get('is_downstream')[0] if('is_downstream' in component) else 'no'
...
def get_input_stacks_vectors_from_graph(inputList):
    ...
    for package,version in inputList.items():
        if package is not None:
            payload = {'gremlin': "g.V().has('pecosystem','NPM').has('pname','"+package+"').has('version','"+version+"').valueMap();"}
            resp = requests.post(_bayesian_graph_url, data=json.dumps(payload))
            resp_json = resp.json()
            #print(resp_json)
            if len(resp_json.get('result').get('data')) > 0:
                ...
                if('code_complexity' in resp_json.get('result').get('data')[0]):
                    code_complexity = float(resp_json.get('result').get('data')[0].get('code_complexity')[0])
                stackMap['LOC'] = LOC
